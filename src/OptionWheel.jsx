...
const OptionWheel = ({
  ...
  const getTargetForIndex = useCallback(index => {
    const cfg = cfgRef.current;
    const cur = targetRef.current;
    const currentIndex = ((Math.round(cur) % cfg.count) + cfg.count) % cfg.count;
  ...
  const isIndexCentered = useCallback(index => {
    const cfg = cfgRef.current;
    if (!cfg.count) return false;

    const targetIndex = ((Math.round(targetRef.current) % cfg.count) + cfg.count) % cfg.count;
    const isSelected = targetIndex === index && selectedRef.current === index;
    const isSettled = Math.abs(posRef.current - targetRef.current) < 0.02;

    return isSelected && isSettled;
  }, []);
  ...
  const handleItemClick = useCallback(
    index => {
      ...
      const cfg = cfgRef.current;
      const nextTarget = getTargetForIndex(index);

      if (!isIndexCentered(index)) {
    ...
    [applyTarget, getTargetForIndex, isIndexCentered]
  );
  ...
  const handleKeyDown = useCallback(
    e => {
      if (e.key === 'Enter') {
        ...
        if (!isIndexCentered(idx)) {
          applyTarget(getTargetForIndex(idx), true);
          return;
        }
  ...
  return (
    <div
      ...
      role="listbox"
      tabIndex={0}
      aria-label="Option wheel. Click an option to center it, then click again or press Enter to open it."
